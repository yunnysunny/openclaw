...
import { getCachedPluginJitiLoader, importWithPluginJiti } from "./jiti-loader-cache.js";

export type PluginSourceLoader = (modulePath: string) => Promise<unknown>;
...
export function createPluginSourceLoader(): PluginSourceLoader {
  ...
  return async (modulePath) => {
    ...
    if (!shouldProfilePluginSourceLoader()) {
      return importWithPluginJiti(jiti, modulePath);
    }
    const startMs = performance.now();
    try {
      return await importWithPluginJiti(jiti, modulePath);
    } finally {
      console.error(
