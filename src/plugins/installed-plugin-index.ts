...
export type InstalledPluginIndexRecord = {
  ...
  source?: string;
  format?: import("./manifest-types.js").PluginFormat;
  bundleFormat?: import("./manifest-types.js").PluginBundleFormat;
  setupSource?: string;
...
export function resolveInstalledPluginIndexPolicyHash(
  input: InstalledPluginIndex | OpenClawConfig | undefined,
): string {
  if (!input) {
    return "";
  }
  if ("policyHash" in input && typeof input.policyHash === "string") {
...
function copySafeInstallRecords(
  ...
  return Object.fromEntries(
    Object.entries(records)
      .filter(([pluginId]) => pluginId.trim() && !isBlockedObjectKey(pluginId))
      .map(([pluginId, record]) => [pluginId, { ...record }]),
  );
}

function buildStartupInfo(record: PluginManifestRecord): NonNullable<InstalledPluginIndexRecord["startup"]> {
  const kind = record.kind;
  const kinds = Array.isArray(kind) ? kind : kind ? [kind] : [];
  return {
    sidecar: record.activation?.onStartup === true,
    memory: kinds.includes("memory"),
...
function buildInstalledPluginIndex(
  ...
  const plugins = registry.plugins.map((record): InstalledPluginIndexRecord => {
    ...
    const installRecord =
      installRecords[record.id] ?? normalizeInstallRecord(params.config?.plugins?.installs?.[record.id]);
    if (installRecord) {
      installRecords[record.id] = installRecord;
    }
...
export const INSTALLED_PLUGIN_INDEX_WARNING = "Generated by OpenClaw — do not edit by hand.";
