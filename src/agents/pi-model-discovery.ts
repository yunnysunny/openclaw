...
import {
  ...
  resolvePiCredentialsForDiscoveryAsync,
...
export async function discoverAuthStorageAsync(
  agentDir: string,
  options?: DiscoverAuthStorageOptions,
): Promise<PiAuthStorage> {
  const credentials = await resolvePiCredentialsForDiscoveryAsync(agentDir, options);
  const authPath = path.join(agentDir, "auth.json");
  if (options?.readOnly !== true) {
